...
def build_network_and_calculate_centrality(edges_df):
    ...
    if n_nodes > 10000:
        ...
        centrality["betweenness"] = nx.betweenness_centrality(G, weight="weight", k=k_samples)
    else:
        # 中小型网络：完整计算
...
def regression_analysis(df):
    ...
    coefficients.to_csv(os.path.join(data_dir, "regression_coefficients.csv"), index=False, encoding="utf-8-sig")
    with open(os.path.join(data_dir, "model_performance.txt"), "w", encoding="utf-8") as f:
